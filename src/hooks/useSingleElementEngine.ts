...
export function useSingleElementEngine({ elementMode, videoSyncMode = "nudge", log }: EngineOptions) {
  ...
  const getDiagnostics = useCallback(() => {
    ...
    return {
      ...
      anchorPaused: null as boolean | null,
  ...
  const removeTrack = useCallback((idOrIndex: string | number) => {
    if (typeof idOrIndex === "number") {
      ...
      setCurrentIndex((prev) => {
        if (idOrIndex < prev) return prev - 1;
        if (idOrIndex === prev)
          return Math.max(0, Math.min(prev, Math.max(0, tracksRef.current.length - 1)));
        return prev;
      });
      return;
    }
...
// Keep old name working for any code that imports useFreezeEngine
export const useFreezeEngine = useSingleElementEngine;
