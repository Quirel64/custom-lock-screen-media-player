...
import type { AnchorMode, SessionOwner } from "../lib/types";
...
const OWNER_LABEL: Record<SessionOwner, { text: string; className: string }> = {
  ...
  anchor: {
    text: "Placeholder hold (track paused, seek bar frozen)",
...
export default function HandoffStatus({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-black/30 p-3 font-mono text-[11px] leading-relaxed text-white/60">
      ...
      {anchorMode === "always-on" && (
        <p className="mt-2 text-[10px] text-rose-300/80">
          always-on: both elements play together — this is the seek-bar snapping bug.
        </p>
      )}
    </div>
  );
