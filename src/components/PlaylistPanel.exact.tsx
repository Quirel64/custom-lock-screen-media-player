  ...
  return (
    <section className="flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur">
      <div className="flex items-center justify-between gap-2 border-b border-white/10 px-4 py-3">
        <div>
          <h2 className="text-sm font-semibold text-white">1. Playlist</h2>
      ...
      {tracks.length === 0 ? (
        ...
        >
          ...
          <p className="max-w-xs text-xs text-white/40">
            Tap to add MP4 / audio files. On iPhone, iOS often only lets you pick
            one file per tap — keep tapping Add to build a real queue. Next/Previous
            on the lock screen need 2+ tracks.
          </p>
        </label>
      ) : (
        <ul className="max-h-[40vh] divide-y divide-white/5 overflow-y-auto">
